...
$(document).ready(function(){
    ...
    $('.filter-item-bar>.filter-date').on('click', 'li', function () {
        ...
        if (!$(this).hasClass("active")) {
            ...
            var ed = $('#end-date').val();
            var path = location.search.split('sd')[0];
            location.href = path + 'sd=' + sd + '&ed=' + ed;
    ...
    $(".filter-item-bar>.filter-area").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            location.href = location.search.replace('aid=' + aid, 'aid=' + new_aid)
    ...
    $(".filter-item-bar>.filter-sort").on("click", "li", function(e) {
        if (!$(this).hasClass("active")) {
            ...
            var new_key = $(this).attr('sort-key');
            alert(new_key);
            location.href = location.search.split('&sk')[0] + '&sk=' + new_key;
...
$(document).ready(function () {
    ...
    $.get('/house/allsearch/' + path, function (data) {
        ...
        if (data.code === 200){
            var house_html = template('house_html_id', {houses: data.hlist});
            $('.house-list').html(house_html);

            var area_id_html = template('area_id_list', {alists: data.alist});
            $('.filter-area').html(area_id_html);
